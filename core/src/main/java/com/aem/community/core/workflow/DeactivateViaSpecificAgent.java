...
@Component(immediate = true, service = WorkflowProcess.class, property = {
		Constants.SERVICE_DESCRIPTION + "=Deactivate AEM Workflow payload via specific replication agent, which is provided as argument in wokflow step",
		Constants.SERVICE_VENDOR + "=Anupam Maiti", "process.label=Deactivate Via Specific Agent" })
public class DeactivateViaSpecificAgent implements WorkflowProcess {
	...
	private static final Logger log = LoggerFactory.getLogger(DeactivateViaSpecificAgent.class);
	...
	public void execute(WorkItem workItem, WorkflowSession workflowSession, MetaDataMap processArguments)
		...
		if (Arrays.asList(getReplicationAgents()).contains(lProcessArgs[0])) {
			...
			try {
				lReplicator.replicate(workflowSession.adaptTo(Session.class), ReplicationActionType.DEACTIVATE,
						lPayloadPath, lReplicationOptions);
			} catch (ReplicationException e) {
				log.error("ReplicationException while deactivating payload, Reason :" + e);
			}
		}
